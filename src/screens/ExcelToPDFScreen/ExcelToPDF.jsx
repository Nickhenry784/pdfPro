...
import ConvertExcelToPDF from '../../axios/ConvertExcelToPDF';
...
const ExcelToPDF = () => {
  ...
  const onClickConvertBtn = async () => {
    ...
    try {
      ...
      const ExcelToPDFPath = `${RNFS.DocumentDirectoryPath}/ExcelToPDF-${dateString}.pdf`;

      const namePDF = `ExcelToPDF-${dateString}.pdf`;
      let response = await ConvertExcelToPDF.post(name, dataBase64);
      if (response !== null) {
        if (setting.sound) {
          sound.play();
        }
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView
        ...
        <Text style={styles.title}>Excel to PDF</Text>
        <Text style={styles.description}>
          Convert Excel documents to PDF files, fast and accurate
        </Text>
        <View style={styles.btnView}>
          {imagesPicker.length === 0 ? (
            <TouchableOpacity
              ...
              <Text style={styles.btnText}>Browse your Excel File</Text>
            </TouchableOpacity>
          ) : (
